...
TEMP_BYTES="/tmp/wget_test_bytes"  # 流量统计文件（新增）
...
echo 0 > "$TEMP_BYTES"    # 初始化流量统计文件（新增）
prev_percentage=0
count=0
...
run_request() {
    ...
    if wget -q -O "$temp_file" --timeout=30 --tries=1 --no-cache "$url" 2>> "$LOG_FILE"; then
        # 获取下载字节数（兼容 macOS 和 Linux）
        if [[ -f "$temp_file" ]]; then
            downloaded_bytes=$(stat -f%z "$temp_file" 2>/dev/null || stat -c%s "$temp_file" 2>/dev/null || echo 0)
